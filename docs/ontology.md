...
- **Derived/materialized state** — reconstructible views such as current claim heads, memory-health snapshots, and context projections.
...
`Event` records that something occurred or entered the system. `Evidence` is a captured observation/source artifact. `Claim` is a proposition derived from evidence, a person statement, a system operation, or other admitted source. `ClaimRelation` expresses `SUPPORTS`, `CORRECTS`, `SUPERSEDES`, `CONTRADICTS`, or `REFINES`. `ResolvedState` is a rebuildable materialized view over claims and relations.

Claims carry both world-valid time (`valid_from`, `valid_to`) and knowledge/record time (`recorded_at`).

## Memory

`MemoryEntry` admits an existing durable artifact into long-term recall. The target may be a claim, event, evidence item, procedure, decision, or work outcome. Memory lifecycle and telemetry are separate from epistemic confidence and retrieval relevance.

`MemoryLifecycleEvent` records creation, seen, used, consolidation, archive, restore, tombstone, and deletion transitions. `MemoryHealthSnapshot` is derived policy output, not canonical truth.

## Context

`ContextRequest` describes what a cognitive computation needs under task, policy, latency, and budget constraints. `ContextItem` references canonical artifacts. `ContextProjection` is an immutable, provenance-bearing, coverage-aware projection pinned to a semantic snapshot/frontier.
